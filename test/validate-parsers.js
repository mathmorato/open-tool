...
if (APP_CONFIG.VERSION !== 'v.2.4.6') {
  console.error('[ERRO] Versão diferente de v.2.4.6');
  process.exit(1);
}
...
if (pkg.version !== '2.4.6' || !/^[0-9]\.[0-9]\.[0-9]$/.test(pkg.version)) {
...
if (!indexHtml.includes('v.2.4.6')) {
  console.error('[ERRO] index.html não contém v.2.4.6');
  process.exit(1);
}
...
if (!readme.includes('v.2.4.6')) {
  console.error('[ERRO] README.md não contém v.2.4.6');
  process.exit(1);
}
...
console.log('[OK] README.md contém cabeçalho v.2.4.6, título # Open Tool e link de acesso online imediato');
