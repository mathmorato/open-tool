...
if (APP_CONFIG.VERSION !== 'v.2.2.2') {
  console.error('[ERRO] Versão diferente de v.2.2.2');
  process.exit(1);
}
...
if (pkg.version !== '2.2.2' || !/^[0-9]\.[0-9]\.[0-9]$/.test(pkg.version)) {
...
if (!indexHtml.includes('v.2.2.2')) {
  console.error('[ERRO] index.html não contém v.2.2.2');
  process.exit(1);
}
...
console.log('[OK] index.html e UI modulares contêm v.2.2.2, layout Open Tool com logo SVG e badge Tamanho do MD');
...
if (!readme.includes('v.2.2.2')) {
  console.error('[ERRO] README.md não contém v.2.2.2');
  process.exit(1);
}
...
console.log('[OK] README.md contém cabeçalho v.2.2.2, título # Open Tool e link de acesso online imediato');
